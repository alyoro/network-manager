  ...
  <v-container>
    ...
    <v-list flat v-for="(item, index) in filteredDevices" :key="index">
      ...
      <v-layout row class="text-md-center align-center">
        ...
        <v-flex xs12 md6>
          <UpdateRoomSocketDialog :device="item" :deviceType="deviceType"/>
        </v-flex>
        ...
        <v-flex xs12 md6>
          <ReportDialog :deviceType="deviceType" :device="item">
            ...
            <div slot="dialog-title">Report for Server</div>
          </ReportDialog>
        </v-flex>
      </v-layout>
    </v-list>
...
import UpdateRoomSocketDialog from "@/components/updateDevices/UpdateRoomSocketDialog.vue";

export default {
  mixins: [searchInfoMixin],
  components: { UpdateRoomSocketDialog }
};
</script>
