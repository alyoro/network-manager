<template>
  <v-dialog v-model="dialog" max-width="1000px" height="500px">
    <template v-slot:activator="{ on }">
      <v-btn v-on="on" flat :dialog="true">UPDATE</v-btn>
    </template>
    <v-card color="white" hide-overlay>
      ...
      <v-layout wrap class="ma-2">
        <v-flex lg6 class="pa-2">
          <v-text-field label="Building" v-model="updatedDevice.building"></v-text-field>
        </v-flex>

        <v-flex lg6 class="pa-2">
          <v-text-field label="Room" v-model="updatedDevice.room"></v-text-field>
        </v-flex>
        ...
        <v-flex lg6 class="pa-2">
          <v-text-field label="Description" v-model="updatedDevice.description"></v-text-field>
        </v-flex>
      </v-layout>
